...
namespace BadNorthBlackSpearman
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        // v1.20: 追踪已处理过的 Squad，每队只克隆一次
        private static HashSet<Squad> _squadsWithClone = new HashSet<Squad>();
        // v1.20: 延迟克隆队列（避免在地图加载阶段生成 → 抽搐）
        private static Queue<Agent> _pendingCloneQueue = new Queue<Agent>();
        private static bool _cloneCoroutineStarted;

        /// <summary>
        /// v1.20: 剑盾兵保持原样出场 + 额外克隆一个黑矛兵加入战场。
        /// 不替换、不占用任何敌方兵种位置。
        /// 克隆延迟到战斗开始后执行，避免在地图预览阶段原地抽搐。
        /// </summary>
        internal static void OnAgentSpawnedHandler(Agent agent)
        {
            ...
            if (ReferenceEquals(squad, null)) return;
            if (_squadsWithClone.Contains(squad)) return;
            _squadsWithClone.Add(squad);

            // 加入延迟队列，等战斗开始后再克隆
            _pendingCloneQueue.Enqueue(agent);
            LogInfo("[SPAWN] Queued BlackSpearman clone for " + agent.name + " (pending: " + _pendingCloneQueue.Count + ")");

            // 启动延迟克隆协程
            if (!_cloneCoroutineStarted && Instance != null)
            {
                _cloneCoroutineStarted = true;
                Instance.StartCoroutine(ProcessCloneQueueDelayed());
            }
        }

        /// <summary>
        /// 延迟处理克隆队列：等战斗真正开始（船靠岸后）再生成黑矛兵。
        /// </summary>
        private static System.Collections.IEnumerator ProcessCloneQueueDelayed()
        {
            // 等 3 秒确保战斗开始、船靠岸
            ...
            int processed = 0;
            while (_pendingCloneQueue.Count > 0)
            {
                var agent = _pendingCloneQueue.Dequeue();
                if (ReferenceEquals(agent, null) || agent == null) continue;
                if (ReferenceEquals(agent.aliveState, null) || !agent.aliveState.active) continue;

                LogInfo("[SPAWN] Cloning BlackSpearman alongside " + agent.name + " (#" + ConvertedAgents.Count + ")");

                try
                {
                    var cloneGO = UnityEngine.Object.Instantiate(agent.gameObject);
                    var cloneAgent = cloneGO.GetComponent<Agent>();
                    if (ReferenceEquals(cloneAgent, null)) { UnityEngine.Object.Destroy(cloneGO); continue; }

                    // 偏移位置到原剑盾兵旁边
                    cloneAgent.transform.position = agent.transform.position + new Vector3(1.2f, 0f, 0.8f);
                    cloneAgent.gameObject.SetActive(true);
                    cloneAgent.dangerous = true;

                    ConvertedAgents.Add(cloneAgent);
                    ApplyBlackSpearman(cloneAgent);
                    AddCloneToSquad(cloneAgent, agent.squad);

                    if (Instance != null) Instance._totalConvertedCount++;
                    processed++;
                }
                catch (Exception ex) { LogErr("Clone: " + ex); }
            }

            if (processed > 0) LogInfo("[SPAWN] Clone queue processed: " + processed + " BlackSpearmans created");
        }

        /// <summary>
        /// 将克隆黑矛兵加入剑盾兵所在的 Squad。
        /// 尝试扩大 squad 容量，失败则设为独立单位。
        /// </summary>
        private static void AddCloneToSquad(Agent clone, Squad squad)
        {
            try
            {
                // 尝试反射扩大 squads 数组容量
                var agentsField = typeof(Squad).GetField("agents",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                var allField = typeof(Squad).GetField("all",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                var manField = typeof(Squad).GetField("men",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

                // 尝试扩大 agents 数组
                if (!ReferenceEquals(agentsField, null))
                {
                    var arr = agentsField.GetValue(squad) as Agent[];
                    if (arr != null)
                    {
                        var newArr = new Agent[arr.Length + 1];
                        Array.Copy(arr, newArr, arr.Length);
                        newArr[arr.Length] = clone;
                        agentsField.SetValue(squad, newArr);
                    }
                }
                if (!ReferenceEquals(allField, null))
                {
                    var arr = allField.GetValue(squad) as Agent[];
                    if (arr != null)
                    {
                        var newArr = new Agent[arr.Length + 1];
                        Array.Copy(arr, newArr, arr.Length);
                        newArr[arr.Length] = clone;
                        allField.SetValue(squad, newArr);
                    }
                }
                if (!ReferenceEquals(manField, null))
                {
                    var arr = manField.GetValue(squad) as Agent[];
                    if (arr != null)
                    {
                        var newArr = new Agent[arr.Length + 1];
                        Array.Copy(arr, newArr, arr.Length);
                        newArr[arr.Length] = clone;
                        manField.SetValue(squad, newArr);
                    }
                }

                // 设置克隆的 squad 引用
                clone.squad = squad;
                LogInfo("[SQUAD] Clone added to squad " + squad.name);
            }
            catch (Exception ex)
            {
                // 失败 → 独立运行（Brain 系统仍可工作）
                LogWarn("[SQUAD] Add failed: " + ex.Message + " — clone will act independently");
            }
        }
