...
---
## 🏗 当前状态（2026-08-17，第四十六轮：重影已解决✅ + 律动保留修复）

> **✅ 重影问题已解决（用户实测确认）**：Mode2（只保留 1 个身体渲染器）后重影消失。
> 根因 = 原版 ColoredCharacter 身体渲染器 **2主+2镜像共 4 层同时绘制**（`Cull Off`）；原版亮色掩盖重叠，**改黑+头盔提亮后显形**。
> **衍生问题与修复**：Mode2 最初保留了"静态主身"（UV 恒 Swordsman0001，不随动画更新）→ 黑矛兵**失去待机律动、完全静止**。
> 第四十六轮修复：**Mode2 改为保留"动画主身"（UV 随帧更新=律动源），禁用静态主身+2镜像** → 无重影 + 律动保留。
