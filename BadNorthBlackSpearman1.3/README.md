...
---
## 🏗 当前状态（2026-08-17，第四十五轮：重影根因定位到"多层渲染叠加"，Mode1 已部署但重影仍残留）

> 用户实测反馈：**重影（头部抽搐/死亡分裂/受击双影）在 Mode1 下仍然存在**。
> 已确认：重影 = 原版 ColoredCharacter 身体渲染器 **2主+2镜像 共 4 层同时绘制**（`Cull Off`）；
> 原版亮色身体掩盖重叠，**改黑+头盔提亮后显形**。Mode1（禁 2 镜像）已证明能修"黑身被洗亮"
> （死亡读屏暗身 px 10→1760、头部窗口扫描最暗=0.06），**但剩余 2 主身仍在叠加 → 重影残留**。
> ✅ **Mode2 已部署（cfg `Diag.SingleBodyMode=2`，代码默认也改 2）**：再禁第 2 主身，**只留 1 个身体渲染器**，待实测。
...
- **🔴 未解决：全局重影**（详见下方"第四十四/四十五轮"）——Mode1 已部署，剩余 2 主身叠加，待 Mode2 验证。
