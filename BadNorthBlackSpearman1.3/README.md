...
---
## 🏗 当前状态（2026-08-17，第四十六轮·闭环 ✅：重影解决 + 律动保留，整改告一段落）

> **⭐ 新需求（地形/建筑感知冲锋）**：黑矛兵"冲刺"技能现在受地形与建筑约束——
> ① **遮挡拦截**：目标锁定格与自身之间的直线必须全程位于主岛可走导航网格上（可走性用游戏权威判定 `NavPos.MoveTo`——IL 确认其返回 `bestDist==0`，海面上的点必然 false），且中段不被房屋（House，完好/燃烧中/**已烧毁残骸**均算）占据（建筑 0.4m 检测半径 + `House.bounds` 世界包围盒兜底，起终点各留 0.4m 余量防贴墙误判）；被遮挡时不发起冲锋（技能保留、交给普通 AI 绕路，0.25s 后重新评估）。
> ② **终点夹回**：目标背靠海面/建筑时，把冲锋距离截短到直线最远可走处——地形边界二分细化后内收 0.2m，至少到达目标锁定格，穿透段夹回**岸上内陆**/建筑前，模型不再冲出海岸、不再浮海漂移。
> ③ **冲锋被阻拦**：冲锋途中当前直线点若已落入不可通过地形/房屋残骸（MoveTo 判定不可达），立即停止进入冷却（不穿墙、不穿水、不穿悬崖）。
> （`SpearChargeComponent.cs`：`IsPointWalkable`(MoveTo) + `IsPointBlockedByHouse` + `IsStraightPathWalkable` + `MaxWalkableDistAlongRay`(二分+内收) + `TryTriggerCharge` 拦截 + `DoWindUp` 夹回 + `DoCharging` 被阻拦）

> **✅ 两个问题已闭环（用户实测确认）**：
> - **重影（头部抽搐/闪白 + 死亡腾空分裂/受击双影）已解决**：根因 = 原版 ColoredCharacter 身体渲染器
>   **2主+2镜像共 4 层同时绘制**（`Cull Off`）；原版亮色身体掩盖重叠，**改黑+头盔提亮后显形**。
> - **待机律动已恢复**：Mode2 保留**动画主身**（UV 随帧更新=律动源），禁用静态主身+2镜像 → 单渲染器无重影 + 动画/律动完整。
> - 方案：cfg `Diag.SingleBodyMode=2`（代码默认 2，已部署 162304B）。

### ✅ 已定稿（可正常游玩）
- **技能**：登岛触发 + 长矛突击（非追踪、可躲、高收益）+ 10s 冷却 + 盾牌格挡 + 技能期可被击杀。
- **闪白根治（用户实测确认）**：根因 = 身体顶点色 **alpha 恒 0**（`Body.SetGrass` 判定短路）→ `AlphaToMask` 丢弃身体 → 身体透明露背景。
  修复 = `BlackSpearmanVisual` 每帧强制身体顶点 **alpha=1 + B=0.02**（不透明黑）。
- **死亡白尸修复**：死亡后 30 帧持续补块（对抗死亡重烘焙清空材质块），尸体保持黑。
- **去剑/长矛**：剑刃帧级擦除 + 部件贴图分区压暗（亮银压暗防剑刃/盾显形、暗灰头盔可见、躯干/手烘黑）；长矛手部纹理压暗。
- **橡皮筋**：冲锋/近战 LateUpdate 硬同步 navPos + 刺击期冻结矛根。
- **✅ 全局重影已解决（第四十六轮）**：Mode2 只保留"动画主身"渲染器（`Diag.SingleBodyMode=2`），消除 4 层叠加且保留待机律动。原两个问题（头部抽搐/闪白 + 死亡腾空分裂/受击双影）均为同一根因（多层渲染叠加、改黑后显形）。

### 🛠 第四十二轮（2026-08-17）：日志整理 + 针对性诊断（已部署 158208B）

- **cfg 新增 `[Diag]` 段**：`VerboseDumps=false`（巨型转储归入 F8/手动）、`HeadTrace=true`（问题①）、`DeathTrace=true`（问题②腾空）、`HitDoubleTrace=true`（问题②受击）。日志从"什么都打"改为**按问题分类可开关**；`[头部采样]` 逐帧行、`[尸体部位]` 全场景扫描、REGISTER/BOOT 重复行等刷屏源已降噪。
- **问题① 头部闪白/抽搐诊断修正**：
  - 头盔采样点改用 **Sprite 真实盔顶**（旧 chestPos+up*0.45 实测全部命中背景亮度 0.62~0.97，永远测不到头盔——R41 关闭 P0 的判据不可靠）；
  - `[头盔统计]` 窗口新增 `暗/亮/交替` 计数：**交替≥3=闪白实锤**、`窗口恒亮且暗=0`=头盔渲染成亮/露背景；
  - 新增 **`[头部·帧擦]`** 非屏幕探测：逐帧统计头部带（帧 relY 0.10~0.50，离线 UV 解码已验证）内"将被擦除"像素，动画帧间 0↔N 交替≥3 即报警；
  - 新增 **`[白帧]`** 转换检测：材质块空 / 网格 4 顶点全零进入与恢复都报警。
- **问题② 死亡影分身/受击两重分身诊断（新增 BlackSpearmanDiagProbe.cs）**：
  - 钩住 **`CorpseManager.AddCorpse`**：黑矛兵静态尸体烘焙瞬间打印 `[腾空·尸体烘焙] 尸体位 vs 身体位 偏移`——**偏移>0.1m 且身体在腾空=双尸实锤**；
  - `[腾空]` 紧凑轨迹每 5 帧一行（身位/抬升/网格塌缩/块状态/第二身/静态尸）；`[分身]` 探测镜像复启用/身体离位/SpriteRenderer 双重渲染三类来源。
- **构建/部署**：0 警告 0 错误，Debug **158208B**（SHA256 `407565BC2967C824…`）已部署 plugins 哈希 MATCH；旧版备份 `.bak_r42`。

### ✅ 第三十轮：两个遗留问题已实施修复（待实测）

**① 持矛的手是蓝色且会与身体脱开（橡皮筋式）**
- 精确分析 `Spear_0/1/2`（128×18）手部区域 y6-13 → 除暖肤（235-348px）外还有**蓝色竖带**（115-120px，**R 41-87 / G 81-119 / B 99-123**，x~17-31）。
- 修复（`DarkenSpearHand`）：暖肤阈值 + **蓝系阈值**（`b>80 && b>r && b>g`）都 ×0.15 重度压暗 → **整只手变黑**，与黑身融合，脱开不可见。

**② 头盔样式被染黑（精确染色）**
- 关键认知：着色器是 **LERP**（b=0.02 时屏幕色≈克隆色）；头盔黑 = 暗灰区 ×0.45(16-45) 与黑躯(25) 对比太小。
- 修复（`GetBrightErasedSprite2`）：暗灰 **×0.45 → ×0.8** → 头盔/肩甲呈可见暗灰(32-80)，躯干仍黑。亮银/暖棕不变。

### ✅ 第三十一轮：第三十轮实测日志分析 + 量化诊断升级（已部署 134144B）

**第三十轮日志结论（无回归）**：
- `DarkenSpearHand` 已执行：Spear_2 压暗 155314px（首个单位打日志，纹理缓存共享）——蓝手压黑路径无误；
- `GetBrightErasedSprite2` 已执行：重压(躯干)=3268 / 中压(头盔灰×0.8)=2103 / 亮银=833 / 残留亮银=0——头盔×0.8 路径无误；
- **无黑矛兵闪白**：`[像素采样]` 45 次 ⚠️偏亮均为**英文兵遮挡/受击白闪**误报（采样点=黑矛兵胸口，英文兵总在其与相机之间；R146/G176/B180≈英文蓝甲）——非黑身问题；
- 死亡补块/冲锋收尾/近战刺击无回归。

**本轮诊断升级（纯日志，无视觉行为改动）**：
- `DarkenSpearHand`：分**暖肤/蓝系**计数 + 蓝手压暗后 max 通道亮度（预期 ≤40=已黑）；
- `GetBrightErasedSprite2`：输出**躯干 avg/max** 与**头盔灰 avg/max**（LERP b=0.02 → 屏幕色≈克隆色，无需被遮挡的屏幕采样即可判定），并拆分**头盔区(帧上半)/肩胸区(帧下半)** 像素数（为"只想头盔灰"隔离决策提供数据）；
- `[像素采样]` 升级为**脚→盔顶垂直条 5 点**：报**最暗点**判定黑身是否正常渲染（≤0.35=✓），盔顶点辅助量化头盔可见性。

### ✅ 第三十二轮：按"部件 y<45"提亮失败（已回退）
- 错误假设"单元 y<45=头盔"→ 把 y<45 全保留 → 帧剑刃（采样单元 y20-24 亮银）被提亮 = 剑柄显现；帧头盔（采样单元 y47-88）没覆盖 = 头盔仍黑。

### ✅ 第三十三轮：反向 UV 映射定位 + 头盔源精准恢复（头盔成功）
...
### 🛠 第四十一轮（用户建议）：头盔标志物诊断（已部署 145408B）

按用户建议用**头盔**（标志性灰银冠饰）做判定物，新增两处诊断：
- **① 头盔变动频率 + 慢放对比（BlackSpearmanVisual.cs）**：`SampleHeadBrightness` 每窗口(30采样≈0.5真实秒)输出
  `[头盔统计#id] 亮度跳变/位移跳变(次/真实秒) + ts=Time.timeScale`；采样计时改 `Time.unscaledDeltaTime`
  （否则慢放会把采样本身一起降速，频率对比失效）。**判定**：跳变率(次/真实秒)不随 ts 下降 = 每渲染帧级变动（渲染层/擦除掩码）；
  随 ts 同降 = 游戏时间（动画帧/状态机）驱动。采样窗口 150→600 次（≈10s，留出"正常→空格慢放→恢复"对比）。
- **② 击杀时头盔计数（SwordRemover.cs）**：死亡当帧结束后读屏，在尸体屏坐标 ±160/±200 窗口内做 4 连通域统计：
  `[击杀头盔计数] 暗身团块/头盔灰团块`——正常=暗身1+头盔灰1；≥2 个暗身 或 ≥2 个头盔灰团块 = **凭空多生成身影/头盔=影分身实锤**。
- **构建**：0 警告 0 错误，Debug **145408B**；已部署 plugins 并哈希验证 MATCH（SHA256 F86B6747…）；旧版备份 `.bak_r40`。

**第四十一轮实测结论（用户日志）**：
1. **P0 头部抽搐 = 不存在/已消除**：站桩时（正常 ts=1.00 与慢放 ts=0.10 均）`[头盔统计] 亮度跳变=0 位移跳变=0`；
   位移跳变率高的窗口全部对应冲锋/近战/后退（角色真实移动），不是抽搐。
2. **P1 死亡影分身 = 新根因（网格顶点归零）**：死亡当帧 `[死亡分裂] 前顶点=(0,0,0)×4 + bounds size=(0.91,100,0.91)` →
   游戏把身体网格顶点全重置为原点 → 身体塌缩不可见 → 当帧 `[击杀头盔计数] 暗身团块=0`（读屏扫不到黑尸）。
3. **诊断改进（第四十一轮·修正版）**：`ScanKillHelmets` 改为**死亡后等 3 帧**（网格重建 + onPreRender 补块生效）再读屏；
   头盔灰阈值 0.20~0.85 → 收紧 0.18~0.55（防岛屿中灰/英文兵误分类）；新增**亮白团块**检测（默认白渲染/白影）。

### 📌 技术沉淀（后续可直接复用）
- `[像素采样]`（WaitForEndOfFrame + ReadPixels 读实际渲染色）——**定位"身体透明/白闪"的决定性手段**；
- `[渲染诊断⚠️]`（持续采样、仅异常打印：`_PartTex=NULL` / `_MainTex` 非克隆 / 顶点 B>0.3 / alpha<0.9）；
- `[死亡分裂]` / `[死亡后]` 转储；`render_frame_uv.py` / `analyze_part_layout.py` / `analyze_spear_hand.py` 离线分析脚本。
...
├── BlackSpearmanDiagProbe.cs   # ★ 第四十二轮：问题②针对性诊断（死亡腾空轨迹/尸体烘焙钩子/受击两重分身）
├── Resources/
