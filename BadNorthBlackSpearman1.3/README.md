...
---
## 🏗 当前状态（2026-08-16，阶段总结·暂告一段落）

> **项目暂停休息**。已完成的成果固化如下；剩余 2 个遗留问题记录在下方，休息后继续研究。
...
- **去剑/长矛**：剑刃帧级擦除 + 部件贴图分区压暗（亮银压暗消除白闪源、暗灰头盔轮廓保留、躯干/手烘黑）；长矛手部纹理压暗。
- **橡皮筋**：冲锋/近战 LateUpdate 硬同步 navPos + 刺击期冻结矛根。

### 🔴 遗留问题（休息后继续，共 2 项）

**① 持矛的手是蓝色且会与身体脱开（橡皮筋式）**
- 现状：黑矛兵挂载的是**我方 Pikeman 的长矛**，长矛精灵（`Spear_0/1/2`，128×18）**自带我方单位的"手"**，该手在部件贴图里是**蓝色调**（非暖肤）。
- 已做的尝试：`BlackSpearmanVisual.DarkenSpearHand` 按"暖肤阈值（r-b>25 且 r>130）"压暗，但**蓝色手不满足暖肤阈值** → 没被压黑，仍显示蓝色。
- "脱开/橡皮筋"：长矛是**独立对象**（挂在 Weapon 锚点），`TrackSpearToHand` 每帧跟随持剑手，但移动/冲锋时矛根与身体的"手"位置仍有偏差，观感=手与身体脱开。
- 后续方向：① 长矛手部改按**蓝色系阈值**压暗（或直接把长矛精灵整体压暗/替换成黑手）；② 研究让长矛严格跟随身体手部锚点（或把长矛手部视觉并入身体）。

**② 头盔样式被染黑（精确染色需研究）**
- 现状：头盔（暗灰区）在分区压暗中按 ×0.45 处理，但配合顶点色 B=0.02 后**观感仍是纯黑**，头盔轮廓/样式看不清。
- 后续方向：精确染色——给头盔单独保留更亮的暗灰（提高 ×系数）、或单独提取头盔区域用原色渲染、或研究 `_PartTex` 按部件精确着色。
