...
---
## 🏗 当前状态（2026-08-16，第三十一轮：第三十轮日志已分析、量化诊断升级已部署）

> 第三十一轮（本日）：分析第三十轮实测日志——两修复均已执行、无黑矛兵闪白回归（`[像素采样]` 亮色系英文兵遮挡误报）；升级量化诊断（压暗后亮度统计 + 脚→盔顶垂直条采样器），**已部署（134144B 哈希 MATCH）**。
...
### 🔴 待实测确认（第三十一轮后）
- ① 蓝手是否变黑 → 日志 `长矛手部压暗 ... 蓝系=N ... 蓝手avg=≤40(max=≤40)` 即为黑；脱开是否不可见 → 肉眼确认；
- ② 头盔/肩甲可见暗灰 → 日志 `头盔灰avg=X(max=Y)` 且 X 明显大于 `躯干avg` 即可见；肩甲/胸甲同属暗灰区会一起变灰——若只想要头盔灰，按已输出的上/下区像素数隔离（头盔在帧上部）；
- ③ `[像素采样]` 最暗点应 ≤0.35（黑身✓）；接战中被英文兵遮挡时整条 >0.35 属正常误报。
